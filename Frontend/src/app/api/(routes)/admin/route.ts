...
import { hasTrustedOrigin, isAdmin, sessionFromRequest } from "../../utils/authUtils";
...
const post = async (request: NextRequest, requestId: string) => {
  ...
  if (!isAdmin(session)) return jsonResult(requestId, failure(403, "Admin access is required."));
