...
import { isAdmin, isGoogleSession, sessionFromRequest } from "../../utils/authUtils";
...
const get = async (request: NextRequest) => {
  ...
  const allowed = isAdmin(session) || (isGoogleSession(session) && listing.merchantId === session.user.uid);
