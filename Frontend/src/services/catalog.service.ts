...
export const listProducts = async (signal?: AbortSignal) => { try { return await postApi<CatalogListData>("/api/catalog", { operation: CatalogOperations.ListProducts }, signal); } catch (error) { throw normalizeApiError(error, "The catalogue could not be loaded."); } };
